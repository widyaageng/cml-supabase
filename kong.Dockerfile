...
# copy conda/mamba nginx default conf and other static files served
# with mamba nginx
RUN mkdir -p /opt/conda/etc/nginx/sites.d
RUN mkdir -p /opt/conda/etc/nginx/default-site/nginx-conda
COPY nginx-mamba-default-site.conf /opt/conda/etc/nginx/sites.d/default-site.conf
COPY index-conda.html /opt/conda/etc/nginx/default-site/nginx-conda/index.html

# kong config dir
RUN mkdir -p /etc/kong-config
RUN chmod  -R 775 /etc/kong-config
RUN chmod -R 775 /opt/conda
RUN chown -R kong:cdsw /etc/kong-config
RUN chown -R kong:cdsw /opt/conda
...
RUN set -ex; \
    ...
    && rm -rf /tmp/kong.deb \
    && chown kong:cdsw /usr/local/bin/kong \
    && chown -R kong:cdsw /usr/local/kong \
